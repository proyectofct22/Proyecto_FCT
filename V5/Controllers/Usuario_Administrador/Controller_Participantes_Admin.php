	...
	if (empty($_SESSION["idUsuario"])) { // Verificamos que el usuario ha iniciado sesión
		...
		if(isset($_POST['SeleccionTorneo'])){
			if(!empty($_POST['torneos'])){
				...
				echo "<script>Swal.fire({ icon: 'error', title: 'Error', text: 'Debe seleccionar un torneo', showConfirmButton: false, })</script>";
			}
		}
		...
		if(isset($_POST['Fechas'])){
			if(!empty($_POST['fecha'])){
				...
				echo "<script>Swal.fire({ icon: 'error', title: 'Error', text: 'Debe seleccionar una fecha', showConfirmButton: false, })</script>";
			}
		}
